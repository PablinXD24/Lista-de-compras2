...
function renderShoppingList() {
    ...
    shoppingList.forEach(item => {
        ...
        const itemValue = document.createElement('input'); // Alterado para usar um input para permitir edição do valor
        ...
        itemValue.addEventListener('input', () => {
            item.value = itemValue.value; // Atualiza o valor do produto na lista de compras
            renderShoppingList(); // Renderiza novamente a lista para exibir o valor atualizado
        });
    });

    // Cálculo do total
    total = shoppingList.reduce((acc, cur) => acc + (parseFloat(cur.quantity) * parseFloat(cur.value || 0)), 0);
    const totalElement = document.createElement('li'); // Cria um elemento para exibir o total
    totalElement.textContent = `Total: R$ ${total.toFixed(2)}`; // Define o texto do total
